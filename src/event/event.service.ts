...
export class EventService {
  async getAllEvent(userId: number) {
    try {
      const allevents = await AppDataSource.getRepository(EventEntity)
        .createQueryBuilder('event')
        .leftJoinAndSelect('event.user_id', 'user')
        .leftJoinAndSelect('event.event_type', 'type')
        .where('event.user_id = :userId', { userId })
        .orderBy('event.when', 'DESC')
        .getMany();

      if (!allevents) {
        throw new NotFoundException(
          'O usuário não possuí nenhum evento vinculado',
        );
      } else {
        return allevents;
      }
  ...
  async getEventById(userid: number, id: number) {
    try {
      const eventbyid = await AppDataSource.getRepository(EventEntity)
        .createQueryBuilder('event')
        .leftJoinAndSelect('event.user_id', 'user')
        .leftJoinAndSelect('event.event_type', 'type')
        .where('event.id = :id', { id: id })
        .andWhere('event.user_id = :userid', { userid: userid })
        .orderBy('event.when', 'DESC')
        .getOne();

      if (!eventbyid) {
        throw new NotFoundException(
          'O usuário não possuí nenhum evento vinculado a esse ID',
        );
      } else {
        return eventbyid;
      }
  ...
  async deleteEvent(userId: number, id: number) {
    try {
      const repository = AppDataSource.getRepository(EventEntity);
