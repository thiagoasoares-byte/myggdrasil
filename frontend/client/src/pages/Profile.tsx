...
import { Loader2, ArrowLeft, User, Trash2, LogOut } from "lucide-react"
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
    ...
    <div className="min-h-screen bg-background">
      ...
      <header className="sticky top-0 z-30 border-b border-border bg-background/90 backdrop-blur-md">
        <div className="flex items-center gap-3 px-5 lg:px-8 py-3.5">
          ...
          <span className="font-serif text-lg font-bold text-foreground tracking-tight">Perfil</span>
        </div>
      </header>
        ...
        <div>
          ...
          <form onSubmit={handleUpdate} className="space-y-4">
            <div className="space-y-1.5">
              <Label htmlFor="profile-name" className="font-mono text-[11px] uppercase tracking-wider">
                Nome
              </Label>
              <Input
                id="profile-name"
            ...
            <div className="space-y-1.5">
              <Label htmlFor="profile-email" className="font-mono text-[11px] uppercase tracking-wider">
                E-mail
              </Label>
              <Input
                id="profile-email"
            ...
            <div className="space-y-1.5">
              <Label htmlFor="profile-birth" className="font-mono text-[11px] uppercase tracking-wider">
                Data de nascimento
              </Label>
              <Input
                id="profile-birth"
            ...
            <div className="flex items-center gap-2 text-xs text-muted-foreground pt-1">
              ...
              <span className={`font-mono ${user?.email_verified ? "text-primary" : "text-destructive"}`}>
                {user?.email_verified ? "verificado" : "não verificado"}
              </span>
            </div>

            <Button type="submit" disabled={loadingUpdate} className="gap-2 h-10 mt-2">
      ...
      <AlertDialog open={deleteOpen} onOpenChange={setDeleteOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle className="font-serif text-lg">Excluir conta</AlertDialogTitle>
            <AlertDialogDescription>
              Esta ação é irreversível. Todos os seus dados, decisões e relações serão permanentemente removidos.
              Confirme digitando sua senha abaixo.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <div className="space-y-1.5">
            <Label htmlFor="delete-password" className="font-mono text-[11px] uppercase tracking-wider">Senha</Label>
            <Input
              id="delete-password"
              type="password"
              value={password}
